...
# Core model
# =====================================================================
# Form C — the paper's master formula (eq:master-wt-general):
#   a_eff = a* · xi · (xi^3 + alpha_0) / (1 + xi^2)
#   xi = sqrt(a_b / a*)
#
# Limits:
#   xi >> 1 (Newtonian):  a_eff -> a* · xi^2 = a_b
#   xi << 1 (deep-MOND):  a_eff -> alpha_0 · sqrt(a_b · a*)
#
# This is the DERIVED formula from shell transport on S^2.
# It differs from the simple crossfade (Form A) in the
# transition and deep-MOND regimes.
# =====================================================================

def crossfade(x, m=1.0):
    """Retained for compatibility; not used by Form C."""
    return 1.0 / (1.0 + x**m)

def wt_predicted_acceleration(a_bar, m=1.0, alpha=1.0):
    """
    Form C master formula:
      a_eff = a* · xi · (xi^3 + alpha) / (1 + xi^2)
    where xi = sqrt(a_b / a*).

    The parameter 'm' is not used in Form C (the crossfade shape
    is fixed by the derivation). Kept in signature for interface
    compatibility.
    """
    a_bar = np.asarray(a_bar, dtype=float)
    xi = np.sqrt(np.clip(a_bar / A_STAR, 0, None))
    return A_STAR * xi * (xi**3 + alpha) / (1.0 + xi**2)

def wt_predicted_acceleration_formA(a_bar, m=1.0, alpha=1.0):
    """Original Form A (simple crossfade), kept for comparison."""
    x = np.asarray(a_bar, dtype=float) / A_STAR
